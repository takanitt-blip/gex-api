...
class ThetaRestAdapter:
    ...
    def _resolve_trade_date(self, as_of: date) -> date:
        ...
        ロジック自体は不変（誤判断36 対応、2026-07 実装で確認・維持）:
        as_of の前日から過去へ遡り、最初に見つかった取引日
        （open/early_close）を返すだけ。この関数はいつ呼ばれるか
        （cron 時刻）を一切知らない。

        ★ このロジックが安全である条件は cron 実行時刻に依存する
        （誤判断36 の学び: 依存する周辺条件をコード側にも明記する）。
        現行の運用前提（update_gex.yml, cron=UTC 06:30 = ET 01:30(EST)
        〜02:30(EDT)）では、cron 発火時点の as_of は「引けの翌暦日」を
        指す。よって as_of の1日前 = 直近の引けた取引日となり、その
